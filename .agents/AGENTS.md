...
- **Task Lifecycle & Roadmap Management (Disparadores `\task`, `\list task` y `\note`)**:
  - El comando `\task` (o `/task`) gestiona el registro y seguimiento de actividades en [`ROADMAP_ACTIVIDADES.md`](file:///d:/User/Documents/Programacion/DnD_Touri_React/ROADMAP_ACTIVIDADES.md).
  - El comando `\list task` (o `/list task`) muestra el resumen estructurado de las tareas de la rama activa, destacando la tarea en foco y las pendientes.
  - El comando `\note` (o `/note`) permite registrar notas, recordatorios y consideraciones técnicas especiales dentro de la actividad activa en [`ROADMAP_ACTIVIDADES.md`](file:///d:/User/Documents/Programacion/DnD_Touri_React/ROADMAP_ACTIVIDADES.md).
  - Si el archivo no existe al momento de invocarse `\task` o `\list task`, el agente lo creará automáticamente con la estructura base del proyecto.
  - **Alineación con la Rama/Feature Activa**: Cada módulo de dominio se desarrolla dentro de su respectiva rama Git de feature (ej. `feature/modulo_de_bastion`). Todas las actividades registradas y priorizadas en `ROADMAP_ACTIVIDADES.md` DEBEN corresponder estrictamente al módulo de la feature activa.
  - **Prohibición Estricta de Auto-Inclusión de Tareas**: El agente **TIENE TERMINANTEMENTE PROHIBIDO** deducir, inventar o agregar tareas por iniciativa propia a `ROADMAP_ACTIVIDADES.md`. Únicamente se registrarán las actividades que el usuario solicite explícitamente mediante `\task`. Si el agente considera útil una tarea complementaria, **debe preguntar primero al usuario** antes de agregarla.
  - **Análisis, Cuestionamiento y Planteamiento Objetivo Previo**: Antes de comenzar el desarrollo o generar el plan formal de cualquier tarea pendiente, el agente **DEBE realizar obligatoriamente un análisis previo y presentar un planteamiento conceptual al usuario**. Este análisis debe abordarse **estrictamente desde un punto de vista objetivo, cuestionando posibles riesgos, casos límite (*edge cases*), impactos en rendimiento y usabilidad, y expresando con claridad su opinión técnica y recomendaciones** sobre la mejor vía de implementación antes de redactar el `implementation_plan.md` o editar código.
  - **Regla Estricta para Completar Tareas**: El agente **NUNCA** marcará una tarea como completada (`[x]`) de manera unilateral. Tras evaluar los cambios y validar compilación (`pnpm tsc --noEmit` / `cargo check`), el agente **DEBE preguntar obligatoriamente al usuario** y esperar su confirmación explícita antes de actualizar la casilla a `[x]`.
